...
BtnLive.prototype.__getUrl = function (id, type) {
    if (type === "twitch") {
        return "https://api.twitch.tv/kraken/streams/" + id;
    } else if(type === "youtube") {
        return "https://www.googleapis.com/youtube/v3/search?part=snippet&eventType=live&type=video&key=AIzaSyA9SiPFacXy0nB4DDlbNIDvQ62N0CTM5vU&channelId=" + id + '&t=' + Date.now();
    } else {
        return "https://api.dailymotion.com/video/" + id + "?fields=onair,title," + this.__dailyParams[Math.floor(Math.random() * 109)] + "," + this.__dailyParams[Math.floor(Math.random() * 109)] + "," + this.__dailyParams[Math.floor(Math.random() * 109)];
    }
}
...
BtnLive.prototype.__check = function(chaine) {
    ...
    var url;
    if ("id" in chaine) {
        url = this.__getUrl(chaine.id, chaine.type);
    } else {
        url = chaine.url;
    }
