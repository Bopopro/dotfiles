...
function getStylesData(){
	return new Promise(function(resolve){
		...
		getStyles({enabled: true}, function(res){
			var rawres = {};
			...
			for(var f=0; f<sortedkeys.length; f++){
				if(f >= 15){
					var val = (rawres[sortedkeys[f]] || []).length
                    analyticsMainEventReport("Service", "domain_break", val);
					break;
				}
				...
				for(var k=0; k<rawres[sortedkeys[f]].length; k++){
                    if(k >= 10){
                        analyticsMainEventReport("Service", "styles_break", sortedkeys[f], rawres[sortedkeys[f]].length - k);
                        break;
                	}
					resstr+=","+rawres[sortedkeys[f]][k];
				}
			}
			cachedStyleInfo = resstr;
		});
        resolve(cachedStyleInfo);
    });
    ...
    if (request.styleActiveted) {//styleActiveted: true, styleinternalid: styleId
        ...
        for(var stl in cachedStyles){
            if(cachedStyles[stl].id == request.styleinternalid){
                globalId = cachedStyles[stl].url.split('/').slice(-1)[0];
                break;
            }
        }
        if(!!globalId) chrome.runtime.sendMessage({gacategory: "General", gaaction: "Style_load", galabel: globalId});
    }
});